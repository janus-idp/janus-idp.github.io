...
This blog will provide information on how we can create a Backstage Frontend plugin as well integrate it with an existing Backstage Backend plugin. The Backend plugin we will use in this article is the Local System Information plugin mentioned in another [blog](https://janus-idp.io/blog/2023/08/23/creating-your-first-backstage-backend-plugin).

We will make some [modifications to the backend plugin](#modify-backstage-backend) to wire it into the Backstage app and then move on to [create a new frontend plugin](#create-frontend-plugin). Once everything is in place and we have verified the frontend plugin integration with the backend plugin, we will then [analyze the code](#analyze-the-code) that makes it possible.
...
- [Created the Local System Information backend plugin](https://janus-idp.io/blog/2023/08/23/creating-your-first-backstage-backend-plugin)
    ...
    - Second table uses `sysInfoCpuColumns` for the columns and `sysInfoData?.cpus` for the CPU data.
      ```tsx showLineNumbers {3,5}
          <Table
            ...
            data={sysInfoData?.cpus || []}
      ```
  - Exports the `SystemInfoPage` component, which contains the Grid containing the above two tables components:
    ```tsx
        export const SystemInfoPage = () => {
          ...
        }
...
- [Run Backstage Showcase locally](https://github.com/janus-idp/backstage-showcase/blob/main/showcase-docs/getting-started.md#running-locally-with-a-basic-configuration)
- [Creating your first Backstage Backend plugin](https://janus-idp.io/blog/2023/08/23/creating-your-first-backstage-backend-plugin)
