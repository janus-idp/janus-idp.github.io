---
title: Production Ready TechDocs in Janus
authors: [jayfray12]
tags: [techdocs]
description: Production Ready TechDocs in Janus using GitHub Actions and OpenShift Data Foundation
---

# Production Ready TechDocs using OpenShift Data Foundation
...
In this blog post, we will show you how to set up an external techdocs builder and publisher using GitHub Actions and the OpenShift Data Foundations operator to create an `ObjectBucketClaim` to mimic an AWS S3 Bucket. This will allow you to generate static HTML documentation from your codebase in a production-ready environment.
...
## Configuring Janus
Once the ODF Operator is installed and an `ObjectBucketClaim` is created, Janus can be configured to use the `ObjectBucketClaim` as the TechDocs publisher.

### Deployment
Update the Janus `Deployment` to include the following:
```yaml
envFrom:
  - configMapRef:
      name: backstage-bucket-claim
