...
*Note: The Backstage Helm chart is also available as an OCI artifact. However, the steps described in this article will focus on the installation from a Helm Chart repository. Instructions on how to leverage the chart from an OCI registry can be found on the chart GitHub project repository.*
...
*Note: The Kubernetes CLI (kubectl) may be desired in order to perform commands against the minikube instance. By default, it is not installed when minikube is installed. Follow [these steps](https://minikube.sigs.k8s.io/docs/handbook/kubectl/) to configure kubectl on the local machine.*
...
```
kubectl port-forward -n backstage svc/backstage
```
