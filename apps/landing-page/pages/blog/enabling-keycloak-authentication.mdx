...
1. Users attempts to access Backstage and is redirected to Keycloak by the OAuth2 proxy
1. User authenticates against Keycloak
1. Upon successful authentication to Keycloak, OAuth process verifies user has met all necessary requirements that are needed to access Backstage
1. Request to Backstage for the processing of the authentication
1. Backstage [Sign In Resolver](https://backstage.io/docs/auth/identity-resolver) ingests request (reading headers provided by the OAuth2 proxy) and either associates the user within an existing entry in the software catalog or a new entry is created   
1. Authentication process is complete and the user can make use of Backstage based on their level of access
...
1. Attempt to locate the user in the Backstage catalog
    1. If found, sign in the user
1. If a user is not found, create a user on the fly and sign them in
...
_Note: If you choose to leverage a realm with a name other than backstage, be sure to substitute the name appropriately throughout the remainder of the article._
...
![MissingImage136](https://user-images.githubusercontent.com/12504403/213074927-deeebb04-51f2-40aa-be64-dd5199826075.png)
...
_Note: The specific configurations provided within this Values file defines a minimum amount of parameters needed to enable the integration between Backstage and Keycloak. It is recommended that the configurations of the OAuth2 proxy be hardened to increase the overall level of security. See the [OAuth2 proxy documentation](https://oauth2-proxy.github.io/oauth2-proxy/docs/configuration/oauth_provider) for the full set of supported options available._
...
![MissingImage222](https://user-images.githubusercontent.com/12504403/213075079-67d23833-60e7-427d-8bec-56e2163ca81f.png)
...
_Note: An [Ingress Controller](https://kubernetes.io/docs/concepts/services-networking/ingress-controllers/) must be present within the cluster in order to properly serve requests destined for Backstage from sources originating outside the cluster._
...
![MissingImage253](https://user-images.githubusercontent.com/12504403/213074707-bd22147a-eea3-4c9d-a6fa-dbc6f875f9a9.png)
...
![MissingImage259](https://user-images.githubusercontent.com/12504403/213074487-74028914-8268-4640-961d-aabe6204d466.png)
