...
In the Janus Backstage chart we provide you with both options. To either use OpenShift `Route` or Kubernetes `Ingress`. The following Helm values can be used to enable and configure the `Route` resource:
    ...
    extraContainers:
      - name: oauth-proxy 
        args:
          ...
          - -cookie-secret="{{ default (randAlpha 32 | lower | b64enc) .Values.global.oauthProxy.cookieSecret }}"
...
Be sure to update the `upstream.ingress.host` property with the desired exposed hostname for Backstage.
...
```bash
helm install -n backstage --create-namespace backstage janus-idp/backstage -f values-backstage-openshift-auth.yaml
```
