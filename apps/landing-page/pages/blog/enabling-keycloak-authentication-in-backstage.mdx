...
```tsx title="packages/app/src/App.tsx" {1,5-7}
...
```ts title="packages/backend/src/plugins/auth.ts" {1,9-40}
...
Update the `global.keycloak.issuerURL` parameter by providing the value that was obtained from the OpenID Endpoint Configuration. The `/.well-known/openid-configuration` portion of the URL can be omitted as it is inferred automatically.

Update the `global.keycloak.clientId` and `global.keycloak.clientSecret` parameters with the values that were obtained from the backstage OAuth client Credentials tab previously.

Next, specify the hostname of the backstage instance by updating the `upstream.ingress.host` parameter.
...
Finally, if there was a desire to make use of a custom Backstage image that was built previously instead of the provided image, update the set of parameters underneath the `upstream.backstage.image parameter`.
...
```bash
helm install -n backstage --create-namespace backstage janus-idp/backstage -f values-backstage-keycloak.yaml
```
