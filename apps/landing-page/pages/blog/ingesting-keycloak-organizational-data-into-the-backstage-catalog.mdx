...
```ts title="packages/backend/src/plugins/catalog.ts" {2,7-17}
// ..
...
Now that both a container image of Backstage containing the necessary components to ingest Keycloak organizational data has been created and Keycloak itself has been configured to enable the Keycloak backend plugin to query the Keycloak API, the final step is to deploy an instance of Backstage to a Kubernetes environment using the [Janus IDP's Backstage Helm chart](https://github.com/janus-idp/helm-backstage/tree/main/charts/backstage).
...
```yaml title="values-backstage-keycloak-plugin.yaml"
upstream:
  backstage:
    image:
      registry: quay.io
    ...
    extraEnvVars:
      ...
      - name: 'APP_CONFIG_catalog_providers_keycloakOrg_default_baseUrl'
        value: '{{ required "Keycloak BaseUrl is Required" .Values.global.keycloak.baseUrl }}'
      - name: 'APP_CONFIG_catalog_providers_keycloakOrg_default_loginRealm'
        value: '{{ required "Keycloak Realm is Required" .Values.global.keycloak.realm }}'
      ...
      - name: oauth2-proxy
        env:
          ...
          - name: OAUTH2_PROXY_OIDC_ISSUER_URL
            value: '{{ required "Keycloak Issuer URL is Required" .Values.global.keycloak.baseUrl }}/realms/{{ required "Keycloak Realm is Required" .Values.global.keycloak.realm }}'
...
```bash
helm upgrade -i -n backstage --create-namespace backstage janus-idp/backstage -f values-backstage-keycloak-plugin.yaml
```
